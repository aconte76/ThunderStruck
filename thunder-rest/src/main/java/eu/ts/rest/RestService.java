...
import eu.ts.rest.server.ModuleVersion;
import eu.ts.rest.server.RestServer;
import eu.ts.rest.server.ServiceDescriptor;
import eu.ts.rest.server.VersionedApi;
import eu.ts.rest.server.filter.AbstractRoute;
import eu.ts.rest.server.filter.HttpMethod;
...
import static eu.ts.rest.server.filter.JsonUtils.toJson;
...
public abstract class RestService extends RestServer {
	...
	private void commonRoutes() throws Exception {
		...
		get("/", (request, response) -> (serviceDescriptor.getDescription() + " is alive"));
	...
	public void module(final RestModule restModule) throws Exception {
		...
		try {
			if (null != restModule) {
				...
				log.info("Register Module: {} (v{})", restModule.getClass().getName(), version.getVersion());
				...
				for (VersionedApi versionedApi : versionSet) {
					if (versionedApi.getBasePath().startsWith(versionBasePath)) {
						throw new Exception(String.format(
								"[%s] Module v%s - '%s' : already registered",
								super.getServiceDescriptor().getDescription(),
								version.getVersion(),
								versionBasePath)
						);
					}
			...
			throw new Exception(String.format("Unable to start %s...", serviceDescriptor.getDescription()), ex);
		}
	}
